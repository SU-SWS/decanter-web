  ...
  <div className="grow">
    ...
    <Section heading="Colors" id="colors">
      ...
      **Color opacity:** To apply opacity to a color, add `/` and a number to the end of the class name, e.g., `bg-digital-red/50` (digital red at 50% opacity), `text-white/20` (white at 20% opacity). See the [Tailwind documentation on changing color opacity](https://tailwindcss.com/docs/text-color#changing-the-opacity).
      ...
      <Deprecated>
        **`foggy`** (including `foggy-light` and `foggy-dark`) is deprecated and will be removed in v8. Use **`fog`** instead — the two colors are identical, e.g., `bg-fog-light` in place of `bg-foggy-light`.

        The **social media brand colors** — `facebook`, `twitter`, `instagram`, `linkedin`, `youtube` — are also deprecated and will be removed in v8. They are not part of the Stanford Identity palette and they go stale as the platforms rebrand (`twitter` predates X). Define the ones you need in your own project config.
      </Deprecated>
    </Section>
    ...
    **Note:** You will still need to import these fonts on your own.
    ...
    - **`font-sans`**\
    **Font stack:** Source Sans 3, ~~Source Sans Pro~~, Helvetica Neue, Helvetica, Arial, sans-serif\
    **Example:** <span className="bg-black-10 px-05em py-01em">The quick brown fox jumps over the lazy dog.</span>
    - **`font-serif`**\
    **Font stack:** Source Serif 4, ~~Source Serif Pro~~, Georgia, Times, Times New Roman, serif\
    ...
    **Example:** <span className="font-mono bg-black-10 px-05em py-01em">The quick brown fox jumps over the lazy dog.</span>
    ...
    <Deprecated>
      **`font-slab`** is removed in v8, along with the class itself — it is not part of the Stanford Identity type system and has no Tailwind core equivalent. If you still need Roboto Slab, define `slab` in your own project config.

      **`font-mono`** keeps working in v8, but Decanter's Roboto Mono stack is removed and the class falls back to Tailwind's default monospace stack. Define `mono` in your own project config if you need Roboto Mono specifically.

      **The `Source Sans Pro` and `Source Serif Pro` fallbacks** in `font-sans` and `font-serif` are removed in v8. The leading families — Source Sans 3 and Source Serif 4 — are unchanged; make sure they are loaded, because the next fallback becomes Helvetica Neue and Georgia respectively.
    </Deprecated>
    </Section>
    ...
    <Deprecated>
      The **`neg2`** and **`neg1`** steps are deprecated and will be removed in v8. Use step `0` or above instead, or a fixed Tailwind spacing utility if you need a value that does not scale.
    </Deprecated>
    ...
    <table>
      ...
      <tbody>
        <tr>
          <td>neg2 <Deprecated inline /></td>
          <td>xs: 0.8rem<br />md: 0.9rem<br />2xl: 1rem</td>
        </tr>
        <tr>
          <td>neg1 <Deprecated inline /></td>
    ...
    **Note:** The HTML root font size is set to 10px in Decanter’s base configuration, thus, **1rem = 10px**.
    ...
    <Deprecated>
      The older em-based font sizes — **`text-m0`** through **`text-m9`**, **`text-09em`** and **`-text-m1`** — are deprecated and will be removed in v8. Use the modular typography classes `type-0` – `type-9` above instead: they are responsive and carry the proportional letter spacing the plain font sizes lack. For a one-off size, use an arbitrary value such as `text-[1.25em]`.

      The pixel-based sizes `text-11` – `text-30` are **not** deprecated.
    </Deprecated>
    </Section>
    ...
    - **`splash`**
    - **`caption`**
    - **`credits`** <Deprecated inline />

    <Deprecated>
      **`credits`** is removed in v8, because small italic text is not recommended for accessibility. To reproduce the v7 styling, use `text-[max(1.6rem,0.9em)] leading-snug italic text-cool-grey`.
    </Deprecated>
    </Section>
    ...
    <Section heading="Accessibility" id="accessibility">
      <Section headingLevel="h3" headingSize={1} heading="Screen readers">
        ...
        <Deprecated>
          **`accessibility-hidden`** and **`a11y-hidden`** were removed in Decanter v7.5.0 and no longer generate any CSS. Use Tailwind's [`aria-hidden:` variant](https://tailwindcss.com/docs/hover-focus-and-other-states#aria-states) instead — `<span class="aria-hidden:hidden" aria-hidden="true">` compiles to the same rule these classes produced.

          **Check your markup before upgrading.** The removal does not break your build — any element that relied on these classes simply becomes visible. Search your templates for `a11y-hidden` and `accessibility-hidden`. Note that these classes hid content from everyone, not just visually; if you want the screen-reader behaviour, use `sr-only` (with `focus:not-sr-only` for skip links).
        </Deprecated>
      </Section>
