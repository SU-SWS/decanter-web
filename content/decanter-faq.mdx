...
Similar to version 6, without installing the Decanter package, one can pull in the compiled Decanter CSS file with the Tailwind CSS classes generated. These classes can then be used in your WordPress theme markup for styling. However, this is not encouraged as the Tailwind CSS generated file is very large (~ 10 MB) in size. Instead, one can either use PurgeCSS along with the compiled CSS file, or install Decanter with Tailwind CSS and pull in the Decanter elements in your Tailwind CSS configuration file to generate only classes that are used in your markup. The most common way to use Tailwind CSS is to use the classes in the markup, but one can also use the @apply method along with your preprocessor of choice.
</Section>
