...
<div className="lg:max-w-600 xl:max-w-800 2xl:max-w-1000">
  <Deprecated>
    ...
    - `text-m2` — use the modular typography class `type-2`, or an arbitrary value such as `text-[1.56em]`.
    - `rs-mt-neg1` — use responsive spacing step `0` or above, or a fixed Tailwind margin utility.
    - `aspect-w-3 aspect-h-2` — these come from the `@tailwindcss/aspect-ratio` plugin, which v8 drops. Use Tailwind's core `aspect-[3/2]` instead.
    - `children:`, `last:children:` — use Tailwind's `*` variant: `*:` and `last:*:`.
  </Deprecated>
